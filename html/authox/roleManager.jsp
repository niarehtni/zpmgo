...
<html>
	...
	-->
  	...
  	String action = "authox/role_addJson.action";
  	String title="新增";
	String disabled = "";
  	if (data != null) {
		...
		action = "authox/role_updateJson.action";
	}
  	RoleItem role= (RoleItem)data;
  	if(role==null){
	  	role=new RoleItem();
 	}
  %>
						...
						<table class="detailTable" border="1">
							<tbody>
								<tr>
									...
									<td width="30%">
										<input type="hidden" name="roleItem.role" value="<%=role.getRole()!=null?role.getRole():"" %>" />
										<input class="null" title="角色名称" name="roleItem.name" value="<%=role.getName()!=null?role.getName():""%>" <%=disabled %>/>
									</td>
								</tr>
							</tbody>
						</table>
