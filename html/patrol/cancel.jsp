...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form action="<%=basePath%>patrol/patrolEvent_listJson.action">
				...
				<tbody>
					<tr>
						<td>
							...
							<%--<input type="hidden" name="patrolEventBean.type"/>--%>
							<%--事件类型<input type="text" name="patrolEventBean.type"/>
							<select type="text" name="patrolEventBean.type">
									<option value=""></option>
									<%
		...
		$(document).ready(function() {
			var p={
	            ...
	            sortorder: 'desc',
	            colModel: [
					{display: '操作', name: 'TYPE', width: 35, sortable: true,
						...
						var handleBtn = "<img title='详情' onclick='top.addTab({href:\""+href+"\",name:\"详情\"});' src='<%=basePath%>/images/jquery/flexgrid/edit.gif' style='cursor:pointer' />";

						href="<%=basePath%>patrol/patrolEvent_cancel.action?type=cancel";
						href+="&patrolEventBean.peid="+row.PRIMARYKEY;
						var cancelBtn = "<img title='作废' onclick='fcancel(event,this)' src='<%=basePath%>/images/jquery/flexgrid/cancel.png' style='cursor:pointer' />";
						return handleBtn+" "+cancelBtn;
						}
					},
	            	{display: '事件编号', name: 'PEID', width: 80, sortable: true},
	            	{display: '事件类型', name: 'TYPE', width: 50, sortable: true},  
	            	{display: '事件状态', name: 'STATE', width: 60, sortable: true},
	            	{display: '二级事件类型', name: 'SECONDTYPE', width: 80, sortable: true},    	    		             
	            	...
	            	{display: '事件标题', name: 'TITLE', width: 100, sortable: true,hide: true},               	    		             
	            	...
	            	**/       	    		             
	            	{display: '备注', name: 'REMARK', width: 400, sortable: true}        	    		             
	            ],
		        detail:{
				...
				,cancel:{
					url:"<%=basePath%>patrol/patrolEvent_cancel.action?type=cancel",
					name:"作废",
					width:1000,
					height:600,
					primaryKey:"patrolEventBean.peids"
		        }
			};
			$("table.listTable").flexigrid(p);	
				
		});		
		function fcancel(e,obj){
			var state = $(obj).parents("tr").find("td[abbr=STATE] div").text();
			//if(state != "作废"){
				jConfirm("该操作将不可恢复，是否确定作废?", "警告信息", function(r) {
					if(r){
					var url = "<%=basePath%>patrol/patrolEvent_cancel.action?type=cancel&patrolEventBean.peids="+$(obj).parents('tr').attr('primarykey');
					submitUrl(url,function(data){
						if(data.result == "error"){
							jAlert("作废失败！","警告信息");
						}else{
							jAlert("作废成功！","警告信息");
							$("table.listTable").flexReload( {});
						}
					},function(){
						jAlert("作废失败！","警告信息");
					});
