...
(function($) {
	$.addFlex = function(t, p) {
		...
		if(p.target){
			documentHeight = $(p.target).height();
			documentWidth = $(p.target).width();
		}
		//标题高度
		var titleHeight = 0;
		if (p.title) {
		...
		var rp = Math.floor(height / rowHeight);
		if(p.colModel){
			var w = 0;
			var cbWidth = p.showCheckBox === false?0:25;
			for(var i=0;i<p.colModel.length;i++){
				if(p.colModel[i].hide){
					continue;
				}
				/**********start:for width of percent**********/
				var cWidth = p.colModel[i].width;
				if(/^\d+(\.\d+)?\%$/.test(cWidth)){
					cWidth = cWidth.slice(0,-1);
					if(!isNaN(cWidth)){
						cWidth = cWidth/100;
						p.colModel[i].width = (width-cbWidth-40)*cWidth-10;  //10为div的padding造成宽度增加，40未防止滚动条造成总宽度减少
					}
				}
				/**********end:for width of percent**********/
				w = w + p.colModel[i].width;
			}
			if(w>width){//出现横向滚动条，行数减1
				rp=rp-1;
			}
		}
		...
		var g = {
					...
					$
							.each(
									...
									function(i, row) {
										...
										if("undefined" == typeof primaryKey){
											primaryKey = row[p.primaryKey];
										}
										$(tr).attr("PRIMARYKEY",primaryKey);
										if (i % 2 && p.striped) {
											tr.className = 'erow';
										}
										if (row.id) {
			...
			init : function(){
				...
				if (p.buttons || p.add || p.detail || p.edit || p.deletes ||p.exportExcel||p.cancel ||p.form||p.likeValueSearch||p.viewConfig) {
					...
					if (p.cancel) {
						if (p.cancel.name == null) {
							p.cancel.name = "作废";
						}
						if (p.cancel.bclass == null) {
							p.cancel.bclass = "cancelBtn";
						}
						btnArrays[btnArrays.length] = {
							name : p.cancel.name,
							bclass : p.cancel.bclass,
							onpress : function() {
								cancel(p.cancel, g);
							}
						};
					...
					if (p.exportExcel) {
						...
						btnArrays[btnArrays.length] = {
							...
							onpress : function() {
								if(!p.exportExcel.url){
									exportExcel(p.exportExcel, g);
								}else{
									var url=p.exportExcel.url;
									if (url.indexOf("?") > 0) {
										url = url + "&time=" + new Date().getTime();
									} else {
										url = url + "?time=" + new Date().getTime();
									}
									var form=p.form.clone();
									form.attr("action",url);
									form.attr("target","_blank");
									
									var ths=$("div.hDiv").find("th:visible");
...
function detail(jsonObj, g, rowExecute) {
	if(rowExecute){
	//行内操作
		var tr = $(g).parents("tr");
	}else{
	...
	if (tr.length == 1) {
		...
		top.addTab({href:url,name:jsonObj.name});
	} else if (tr.length > 1) {
...
function cancel(jsonObj, g) {//作废
	...
	jConfirm("该操作将不可恢复，是否确定作废" + tr.length + "条记录", "警告信息", function(r) {
		if (r) {
			var url = jsonObj.url;
			if (url.indexOf("?") > 0) {
			...
			$(g.bDiv).find("tr.trSelected").each(
					function() {
						url += "&" + jsonObj.primaryKey + "="
								+ encodeURI($(this).attr("primaryKey"));
					});
			$.ajax( {
				type : 'post',
				url : url,
...
function edit(jsonObj, g, rowExecute) {
	if(rowExecute){
	//行内操作
		var tr = $(g).parents("tr");
	}else{
...
function deletes(jsonObj, g, rowExecute) {
	if(rowExecute){
	//行内操作
		var tr = $(g).parents("tr");
	}else{
...
function executeRow(jsonObj, obj, noop) {
...
function executeRows(jsonObj, g, noop) {
	var tr = $(g.bDiv).find("tr.trSelected");
	jsonObj = $.extend( {
		url : false,
		primaryKey : false,
