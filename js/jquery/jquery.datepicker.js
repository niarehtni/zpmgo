/**
 * 
 * Date picker Author: Stefan Petre www.eyecon.ro Download: http://www.jb51.net
 */
$(document).ready(
		function() {
			...
			$("input.date").each(
					function() {
						var a = $(this);
						if (a.val() == '' && a.readonly != 'readonly'
								&& a.disabled != 'disabled') {
							if(a.attr("default")=='day'){
								var now = new Date();
								...
								a.val(now.getFullYear()+"-"+month+"-"+day);
							}
						}
						a.DatePicker( {
							format : 'Y-m-d',
			...
			$("input.month").each(
					function() {
						var b = $(this);
						if (b.val() == '' && b.readonly != 'readonly'
								&& b.disabled != 'disabled') {
							var datetime = new Date();
							var now = datetime.getFullYear() + "-"
									+ (datetime.getMonth() - 1);
			...
			$("select.year").each(function(){
				selectYear($(this).get(0), now.getFullYear());
			});
			$("select.month").each(function(){
				selectMonth($(this).get(0));
			});
			
			$("select.year[default='now']").each(function(){
				setYear($(this).get(0), now.getFullYear());
			});
			$("select.month[default='now']").each(function(){
				setMonth($(this).get(0), now.getMonth()+1);
			});
		});

var Lastyear;
function selectYear(selObj, Cyear) {
	var len = 16 // select长度
...
function selectMonth(obj) {
	...
	for (i = 0; i < len; i++) {
		...
		newOpt.text = (1 + i) + "月";
		newOpt.value = (1 + i);
		obj.options.add(newOpt, i);
	}
}
function setYear(obj,year){
	$(obj).val(year);
}
...
(function($) {
	var DatePicker = function() {
		var ids = {}, tpl = {
			...
			return new Date(y || now.getFullYear(), m || now.getMonth(), d
					|| now.getDate(), h || now.getHours(), min
					|| now.getMinutes(), 0);
			...
			if (el.is('a')) {
				...
				if (parentEl.is('th')) {
					...
					case 'datepickerViewMonths':
						...
						options.current.addMonths(Math
								.floor(options.calendars / 2)
								- tblIndex);
						tblEl.get(0).className = 'datepickerViewDays';
						break;
					case 'datepickerViewYears':
						...
						.each(function() {
							if (!$(this).data('datepicker')) {
									...
									if (options.date.constructor != Array) {
										...
										for ( var i = 0; i < options.date.length; i++) {
											options.date[i] = (parseDate(
													options.date[i],
													options.format).setHours(0,
													0, 0, 0)).valueOf();
										}
										if (options.mode == 'range') {
											options.date[1] = ((new Date(
													options.date[1])).setHours(
													23, 59, 59, 0)).valueOf();
										}
									}
								} else {
								...
								if (!options.current) {
									...
									options.current = parseDate(
											options.current, options.format);
								}
								...
								var cal = $(tpl.wrapper).attr('id', id).bind(
										'click', click).data('datepicker',
										options);
								...
								cal.find('tr:first table').addClass(
										'datepickerViewDays');
								fill(cal.get(0));
								if (options.flat) {
									cal.appendTo(this).show().css('position',
											'relative');
									layout(cal.get(0));
								} else {
